...
namespace DeepSound.Helpers.Utils
{
    ...
    {
        //########################## IMethods Application Version 2.6 ##########################
    }
...
#pragma warning restore 618
        ...
        public static void DisplayReportResult(Activity activityContext, dynamic respond)
        {
            ...
            {
                errorText = error.Error.Replace("&#039;", "'");
            }
            else if (respond is MessageObject errorRespond)
            {
                errorText = errorRespond.Message;
            }
            else if (respond is MessageDataObject messageData)
            {
                errorText = messageData.Data;
            }
            else
            {
        ...
        public class AudioRecorderAndPlayer
        {
            ...
            private static Android.Media.MediaPlayer PlayerStatic { get; set; }
            ...
            public AudioRecorderAndPlayer(string userId)
            {
                try
                {
                    ...
                    SoundFileFullPath = new File(Path.FolderDcimSound + "/" + userId + "/" + SoundFile);

                    var dir = Path.FolderDcimSound + "/" + userId;
            ...
            public void StartRecourding()
            {
                try
                {
                    ...
                    {
                        Console.WriteLine("start prepare() failed" + e);
                    }
            ...
            {
                var soundFileFullPath = Path.FolderDcimMyApp + "/" + folderName + "/" + soundFile;
        ...
        {
            public static void Save_Images_CostomName(string savedfoldername, string fileUrl, string typeimage,
                string imageid)
            {
                try
                {
            ...
            public static string CopyMediaFileTo(string pathOfFile, string toFolderName,
                bool saveOnPersonalFolder = true, bool saveOnGallaryFolder = false)
            {
            ...
            public static Bitmap Retrieve_VideoFrame_AsBitmap(Context context, string mediaFile)
            {
                try
                {
                    Bitmap bitmap;
                    ...
                    {
                        bitmap = ThumbnailUtils.CreateVideoThumbnail(new File(mediaFile), new Size(200, 200), new CancellationSignal());
                        return bitmap;
                    }
                }
                catch (Exception exception)
        ...
        public static class PhoneContactManager
        {
            ...
            public static IEnumerable<UserContact> GetAllContacts()
            {
                ...
                using (var phones =
                    Application.Context.ContentResolver.Query(ContactsContract.CommonDataKinds.Phone.ContentUri, null,
                        null, null, null))
                {
                    if (phones != null)
                    {
            ...
            public static UserContact Get_ContactInfoBy_Id(string fromUriId)
            {
                try
                {
                    ...
                    var contacts = Application.Context.ContentResolver.Query(
                        ContactsContract.CommonDataKinds.Phone.ContentUri, null, "_id = ?", new[] { fromUriId }, null);
                    if (contacts != null)
                    {
            ...
            public static void InsertContact(string fisrtName, string lastName, string number, string email,
                string company)
            {
                List<ContentProviderOperation> contents = new List<ContentProviderOperation>();
                try
                {
        ...
        public static class Time
        {
            ...
            public static string TimeAgo(int time, bool withReplace)
            {
                try
                {
            ...
            public static string ReplaceTime(string time)
            {
                ...
                {
                    time = time.Replace("minutes ago", LblCutMinutes).Replace("minutes ago", LblCutMinutes);
                }
        ...
        public static class App
        {
            ...
            public static string GetKeyHashesConfigured(Context applicationContext)
            {
                try
                {
                    PackageInfo info = applicationContext.PackageManager.GetPackageInfo(applicationContext.PackageName,
            ...
            public static string GetValueFromManifest(Context applicationContext, string nameValue)
            {
                ...
                {
                    ApplicationInfo ai =
                        applicationContext.PackageManager.GetApplicationInfo(applicationContext.PackageName,
                ...
                try
                {
                    if (AppSettings.ShowNotification)
                    {
                        ...
                        NotificationCompat.Builder builder =
                            new NotificationCompat.Builder(Application.Context, notificationId)
                        ...
                        try
                        {
                            ...
                            string filePath = System.IO.Path.Combine(
                                Environment.GetFolderPath(Environment.SpecialFolder.Personal), "mmSavedVideos");
                            string mediaFile = filePath + "/" + filename;
        ...
        public static class Path
        {
            ...
            public static string AndroidDcimFolder = GetDirectoryDcim();
            ...
            private static string GetDirectoryDcim()
            {
                try
                {
                    var directories1 = Application.Context.GetExternalFilesDirs(null); //storage/emulated/0/Android/data/com.wowondermessenger.app/files
                    if (directories1.Length > 0)
                    {
                        var pathDefault = directories1[0].AbsolutePath.Split("/Android/")?.FirstOrDefault();
